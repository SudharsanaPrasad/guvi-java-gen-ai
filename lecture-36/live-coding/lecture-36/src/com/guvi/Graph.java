 ...
 */
public class Graph {
    // If n = 5; the values of nodes can be from 0-5
    private int n;
     ...
     *  - dfsTraversal(start) covers only what is reachable from 'start'
     *  - Since a disconnected graph has multiple islands, we need to loop
     *      over all node to ensure we start a traversal once per an island
     */
    public int countConnectedComponents() {
        // directed vs undirected? we'll support only an undirected graph
        if(directed) {
            throw new IllegalStateException("Connected components supports only undirected graphs");
        }
        // undirected:
        // need an array of visited nodes
        boolean[] visited = new boolean[n];
        // need to track the number of components
        int components = 0;
    ...
    /**
     * dfsMark - only for marking reachability inside a single component
     * Input: node, visited array
     */
    private void dfsMark(int node, boolean[] visited) {
