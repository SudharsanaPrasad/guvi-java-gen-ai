...
You must use:
	•	mongosh for database + query work
...
Note: In MongoDB, collections are created automatically when you insert the first document.
...
Dataset requirements:
	•	At least 2 restaurants in Bangalore
	•	At least 1 restaurant in Hyderabad
	•	At least 1 restaurant that has rating >= 4.5
	•	At least 2 restaurants that share at least one cuisine (e.g., both offer "South Indian")
...
items must look like this (structure requirement):
...
Dataset requirements:
	•	At least 3 different statuses across orders: PLACED, PREPARING, DELIVERED, CANCELLED
...
Return only: name, area, cuisines, rating (hide _id).
	6.	Find restaurants that serve "South Indian" and have rating >= 4.0.
...
Part G — Compass Verification
	18.	Open MongoDB Compass, connect to your local MongoDB instance, and verify:

	•	The restaurants collection contains your inserted data
	•	The orders collection contains your inserted data
	•	You can visually confirm at least one nested items array inside an order document
